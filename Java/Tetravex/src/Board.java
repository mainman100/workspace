...
public class Board extends JPanel
{
	...
	private ImageIcon picture=test.picture;
	...
	private int elapsedTime,startTime;
	...
	{
		for(int i=0;i<size;i++)
			for(int j=0;j<size;j++)//Re-fill the mixed board
			{
				mixedBoard[i][j]=solvedBoard[i][j];
				evaluatedBoard[i][j]=null;
			}
		lablabla();//Mix it again
		Graph g=new Graph();//Solve using AI
		g.graphSearch(this);
		for(int i=0;i<size;i++)
			for(int j=0;j<size;j++)
			{
				evaluatedBoard[i][j]=mixedBoard[i][j];
				mixedBoard[i][j]=null;
			}
		repaint();
	}
