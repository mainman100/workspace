...
public class Main extends JFrame {
	...
	public void play() {
		...
		try {
			...
			int n = savesList.size();// As the list size will change with
										// removal
		...
		try { // Get high scores
			...
			if (!file.exists())
				throw new FileNotFoundException();// It will be catched and
													// write
			...
			try {
				...
				// and an empty list of five empty arrays will be
				// written
		...
		// **************************************************
		setLayout(null);
		// JLabel l=new JLabel(new ImageIcon("ME.jpg"));
		// l.setBounds(300,0,604,453);
		// add(l);
	...
	public void paint(Graphics gd)// Paint Tetravex
	{
		...
		if (c == null && load == false) {
			GradientPaint gradient = new GradientPaint(400, 500, Color.RED,
					600, 400, new Color(0, 200, 0));
			g.setPaint(gradient);
	...
	public void save(String saveName) {
		try {
			String fileName = "Saves\\" + saveName + ".ser";// Make the file in
															// the saves folder
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (command.equals("save") && c != null)// showWindow
			{
				...
				if (name != null) {
					...
					&& command.substring(0, 7).equals("newGame")) {
				...
				if (c != null)// If it is not the first time,remove the old
								// board
								// and stop the timer
				{
					remove(c);
				...
				repaint();
					...
					&& command.substring(0, 11).equals("High scores")) {// show
																		// high
																		// scores
	...
	public void showHighScores(int n) {
		...
		for (int i = 0; i < 5 && p[i].getTime() != -1; i++)
			data += p[i] + "\n";
		JOptionPane.showMessageDialog(null, data, "High scores for size " + n
