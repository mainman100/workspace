...
public class Board extends JPanel
{
	...
	private int count,x,y,deltaX,deltaY;
	public Board(int n)
	{
		size=n;
	...
	public square peek()
	{
		int out=(count-1)/size;
		int rem=(count-1)%size;
		return mixedBoard[out][rem];
	}
	public square pop()
	{
		int out=(count-1)/size;
		int rem=(count-1)%size;
		square temp=mixedBoard[out][rem];
		mixedBoard[out][rem]=null;
		return temp;
	}
	public boolean push(square q)
	{
		count++;
		int out=(count-1)/size;
		int rem=(count-1)%size;
		...
		int j=rem;
		if(i!=0)
			if(mixedBoard[i-1][j].getDown()!=q.getUp())
				return false;
		if(j!=0)
			if(mixedBoard[i][j-1].getLeft()!=q.getRight())
				return false;
		mixedBoard[out][rem]=q;
		return true;
	}
	...
	private void constructGUI()
	{
		...
		timeLabel.setBounds(size*tileSize+65,0,50,20);
